...
function getDeployHelp() {
    const boldFn = (str) => clc.bold(str);
    let targetHelp = "Deploy targets include:\n\n";
    for (const [targetName, target] of Object.entries(deploy_1.TARGETS)) {
        const desc = target.help || `Deploy ${targetName} resources`;
...
exports.command
    ...
    .before((options) => {
    options.filteredTargets = (0, filterTargets_1.filterTargets)(options, [...deploy_1.VALID_DEPLOY_TARGETS]);
    const permissions = options.filteredTargets.reduce((perms, target) => {
        return perms.concat(deploy_1.TARGET_PERMISSIONS[target]);
    }, []);
